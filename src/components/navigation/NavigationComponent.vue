<template>
  <div>
    <menu-icon v-if="!showMenu" class="h-6 w-6 min-[945px]:hidden cursor-pointer" @click="toggleMenu" />

    <nav class="flex py-2 px-6 gap-8 rounded-full border border-nw-border bg-[#160D2F] bg-opacity-20 max-[944px]:hidden" id="menu">
    ...
    <div v-if="showMenu" class="absolute top-0 bottom-0 left-0 right-0 h-full w-full z-10 bg-nw-dark-blue">
      <x v-if="showMenu" class="absolute h-6 w-6 min-[946px]:hidden z-10 top-10 right-10 cursor-pointer" @click="toggleMenu" />
